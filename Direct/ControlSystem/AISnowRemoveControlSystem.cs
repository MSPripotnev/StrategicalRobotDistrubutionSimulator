...
public class AISnowRemoveControlSystem : IPlanningControlSystem {
    ...
    public AISnowRemoveControlSystem(int _seed = 0) {
        ...
        /// desire of working at this road
    ...
    public SystemAction[] PlanPrepare(AgentStation station, TacticalMap map, DateTime time, bool repeat = false) {
        StrategicQualifier.Qualify(station, map.Stations.OfType<Meteostation>().First(),
            station.AssignedRoads, DeviceQualifier, WorkQualifier, out var agentToDeviceQualifies, out var agentToRoadQualifies);
    ...
    public void RecordActionQualify(SystemAction action, bool finished = false) {
        if ((action.Type == ActionType.WorkOn || action.Type == ActionType.ChangeDevice) && !StrategyQualifyRecordings.ContainsKey(action)) {
            StrategyQualifyRecordings.Add(action, new StrategyTaskQualifyReading());
            StrategyQualifyRecordings[action].TaskTime = action.EndTime - action.StartTime;
            if (action.Type == ActionType.WorkOn) {
                if (action.Subject is not SnowRemover remover || action.Object is not Road road) return;
                if (!finished) {
                    StrategyQualifyRecordings[action].FuelCost = remover.Fuel;
                    StrategyQualifyRecordings[action].DeicingCost = remover.DeicingConsumption;
                    StrategyQualifyRecordings[action].RemovedSnow = road.SnownessRemoved;
                    StrategyQualifyRecordings[action].RemovedIcy = road.IcyPercent;
                    if (remover.Home?.PlannerModule is AISnowRemoveControlSystem aiSystem)
                        // StrategicQualifier.Qualify(remover, road, )
                } else {
                    StrategyQualifyRecordings[action].FuelCost -= remover.Fuel;
                    StrategyQualifyRecordings[action].DeicingCost = remover.DeicingConsumption - StrategyQualifyRecordings[action].DeicingCost;
                    StrategyQualifyRecordings[action].RemovedSnow = road.SnownessRemoved - StrategyQualifyRecordings[action].RemovedSnow;
                    StrategyQualifyRecordings[action].RemovedIcy -= road.IcyPercent;
                }
            }
        }
